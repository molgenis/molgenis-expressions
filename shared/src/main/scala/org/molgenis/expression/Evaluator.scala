...
object Evaluator {
  ...
  def getVariables(expression: Expression): Set[String] = {
    ...
    expression match {
      ...
      case BinaryOperation(_, left, right) =>
  ...
  def evaluateSetOp(left: Set[Any], right: Set[Any], op: SetOperator): Boolean =
    op match {
    ...
    private def handleArithmetic(
      ...
      (left, right) match {
        case (l: Number, r: Number) =>
          arithmetic(op, l.doubleValue(), r.doubleValue())
        case _ =>
          Failure(new IllegalArgumentException(s"Cannot $op $left and $right"))
      }

    private def handleComparison(
        op: ComparisonOperator,
        left: Any,
        right: Any
    ): Try[Boolean] =
      (left, right, op) match {
        case (l: Number, r: Number, Less) =>
          Success(l.doubleValue() < r.doubleValue())
        case (l: Number, r: Number, LessOrEqual) =>
          Success(l.doubleValue() <= r.doubleValue())
        case (l: Number, r: Number, GreaterOrEqual) =>
          Success(l.doubleValue() >= r.doubleValue())
        case (l: Number, r: Number, Greater) =>
          Success(l.doubleValue() > r.doubleValue())
        ...
        case _ =>
          ...
          op match {
            case op: BooleanOperator =>
              handleBoolean(op, isTruthy(left.get), isTruthy(right.get))
            case op: SetOperator => handleSet(op, left.get, right.get)
            case op: ArithmeticOperator =>
              handleArithmetic(op, left.get, right.get)
            case op: ComparisonOperator =>
              handleComparison(op, left.get, right.get)
          }
      }
    }
      ...
      evaluate(operand) match {
        ...
        case Success(value) =>
          (op, value) match {
            ...
            case _ =>
              Failure(new IllegalArgumentException(s"Cannot $op $value"))
          }
      }
    }

    def evaluate(expression: Expression): Try[Any] =
      expression match {
        ...
        case BinaryOperation(binaryOp, leftExpr, rightExpr) =>
          handleBinary(binaryOp, leftExpr, rightExpr)
